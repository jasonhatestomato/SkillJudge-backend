...
当前最优先工作不是继续扩邮件，而是完成“预创建 scorer -> 预分配 -> task 内确认”的联调闭环：
...
1. 联调 `POST /api/v1/scorers/provision`
2. 联调新建 task 第 4 步弹窗邀请后刷新评分员下拉
3. 联调创建 task 完成后批量 `notify` 预分配评分员
4. 验证评分员端待评分列表只出现 `accepted` 任务
5. 验证教师端对未完成视频的重新分配调整
...
1. 前端新建任务页改用弹窗即时预创建评分员
2. 创建任务后只对本 task 预分配评分员发站内通知
3. 教师端确认“未完成视频可调整、已完成不调整”的边界

说明：

- 邮件发送失败不会回滚已写入的关系和 invitation 记录。
- 首次建号但邮件失败时，补发会重新生成临时密码。
...
2. 评分员消息中心 / 通知列表
3. 教师端“再次通知”反馈
4. invitation 状态刷新与失败提示

### 8.6 实施顺序

1. 学校级 `provision`：先让新评分员立即进入 scorer pool
2. 创建 task 后按已选评分员执行 `notify`
3. 评分员端仅展示 `accepted` 任务
4. 教师端调整未完成视频分配
5. 视反馈补充任务详情页与消息状态刷新
